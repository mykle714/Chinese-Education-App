import { apiGet, apiPost, apiPatch, apiDelete, withFallback } from '../../api/http';

// Every call goes through src/api/http.ts (base URL, Authorization read fresh at call time,
// `credentials: 'include'`, throw-on-non-2xx); `withFallback` supplies this module's wording
// for an error body that carries no `error` field. See docs/ARCHITECTURE_REVIEW.md finding 5.
// Paths must stay in step with server/routes/nightMarketSandboxRoutes.ts.
...
export async function listSandboxPlacements(): Promise<SandboxPlacement[]> {
  const data = await withFallback(
    apiGet<{ placements?: SandboxPlacement[] }>('/api/nightMarketSandbox'),
    'Failed to list sandbox placements',
  );
...
export async function addSandboxPlacement(input: {
  ...
  const data = await withFallback(
    apiPost<{ placement: SandboxPlacement }>('/api/nightMarketSandbox', input),
    'Failed to add sandbox placement',
  );
...
export async function moveSandboxPlacement(
  ...
  const data = await withFallback(
    apiPatch<{ placement: SandboxPlacement }>(
      `/api/nightMarketSandbox/${encodeURIComponent(id)}/position`,
      { offsetCol, offsetRow },
    ),
    'Failed to move sandbox placement',
  );
  return data.placement;
}
...
export async function setSandboxPlacementVersion(id: string, activeVersion: number): Promise<SandboxPlacement> {
  const data = await withFallback(
    apiPatch<{ placement: SandboxPlacement }>(
      `/api/nightMarketSandbox/${encodeURIComponent(id)}/version`,
      { activeVersion },
    ),
    'Failed to set sandbox placement version',
  );
  return data.placement;
}
...
export async function setSandboxPlacementLock(id: string, locked: boolean): Promise<SandboxPlacement> {
  const data = await withFallback(
    apiPatch<{ placement: SandboxPlacement }>(
      `/api/nightMarketSandbox/${encodeURIComponent(id)}/lock`,
      { locked },
    ),
    'Failed to set sandbox placement lock',
  );
  return data.placement;
}
...
export async function setSandboxPlacementSettings(id: string, settings: SandboxSettings): Promise<SandboxPlacement> {
  const data = await withFallback(
    apiPatch<{ placement: SandboxPlacement }>(
      `/api/nightMarketSandbox/${encodeURIComponent(id)}/settings`,
      { settings },
    ),
    'Failed to set sandbox placement settings',
  );
  return data.placement;
}
...
export async function iterateSandboxPlacement(): Promise<SandboxPlacement | null> {
  const data = await withFallback(
    apiPost<{ trace?: unknown; placement?: SandboxPlacement | null }>('/api/nightMarketSandbox/iterate'),
    'Failed to iterate the sandbox',
  );
...
export async function clearSandboxPlacements(): Promise<{ deleted: number; placement: SandboxPlacement }> {
  const data = await withFallback(
    apiDelete<{ deleted?: number; placement: SandboxPlacement }>('/api/nightMarketSandbox'),
    'Failed to clear sandbox',
  );
...
export async function removeSandboxPlacement(id: string): Promise<void> {
  await withFallback(
    apiDelete<unknown>(`/api/nightMarketSandbox/${encodeURIComponent(id)}`),
    'Failed to delete sandbox placement',
  );
}
