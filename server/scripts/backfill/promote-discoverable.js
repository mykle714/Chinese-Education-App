 ...
 *
 * THE BAR = `buildCompletePredicate` (every applicable manifest step stamped at its
 ...
 * caller sees a per-word verdict (promoted / already discoverable / not ready + why).
 * Without: any not-yet-discoverable row that already CLEARS the bar — i.e. rows some
 * pipeline fully enriched but nobody promoted (the self-heal sweep for the backlog).
 */
...
async function main() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!isDiscoverableReady(row, approved)) {
        notReady++;
        // Name the exact blocker so the caller re-runs the right manifest step
      ...
      if (rowCount === 1) {
        promoted++;
        console.log(`  ✅ ${row.word1} (id ${row.id}) — discoverable (difficulty ${row.difficulty})`);
      } else {
        notReady++;
...
main().catch((err) => {
  console.error('❌ promote-discoverable failed:', err);
  process.exit(1);
});
