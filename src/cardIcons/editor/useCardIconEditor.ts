 ...
 */
export function useCardIconEditor({ currentEntry, nextEntry }: UseCardIconEditorParams) {
    ...
    }, [displayCurrentEntry, setAdvDraftBoth]);
    ...
    const handleSaveLayout = useCallback(async () => {
        ...
        try {
            ...
            const res = await saveIconLayout(currentEntry.id, draftLayout, snapConfig, textColors, textLayout, cardColor);
    ...
    }, [currentEntry, draftLayout, advMode, textDraft, exitEdit, snapMove, snapRotate, snapResize, textForeign, textEnglish, cardColor]);
    ...
    const handleResetConfirmed = useCallback(async () => {
        ...
        try {
            ...
            await saveIconLayout(currentEntry.id, null, null, null, null, null);
    ...
    const persistSelectedSense = useCallback((entry: VocabEntry, sense: string | null) => {
        ...
        saveSelectedSense(entry.id, sense).catch((err) => {
        ...
        // No deps: saveSelectedSense reads the bearer token itself at call time (via
        // src/api/http.ts → authHeader), so this callback's identity no longer churns
        // every ~15 minutes when the access token rotates. See CLAUDE.md "Never
