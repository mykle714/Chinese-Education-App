 ...
 *   x = 0.5, y ∈ DEFAULT_PLACEMENT_YS, scale ∈ DEFAULT_PLACEMENT_SCALES, rotation = 0, flipX falsy.
 *
 ...
 *  `src/cardIcons/cardIconLayout.ts`; the two must stay in sync. */
export const DEFAULT_PLACEMENT_SCALES = [1.25, 1.2, 1] as const;

/** Y centers that count as a "default placement" — the current grid-aligned default
 *  (10 × 0.05 × 295/426 ≈ 0.34624) plus the legacy 0.3333 that basic swaps wrote before the
 *  grid alignment. Mirror of `DEFAULT_PLACEMENT_YS` in the client's `src/cardIcons/cardIconLayout.ts`;
 *  the two must stay in sync. */
export const DEFAULT_PLACEMENT_YS = [10 * (0.05 * (295 / 426)), 0.3333] as const;
...
export const IS_ADVANCED_LAYOUT = `(
  ...
  AND (
    ...
    OR NOT (
      ...
      AND (ve."iconLayout"->0->>'y')::float IN (${DEFAULT_PLACEMENT_YS.join(", ")})
