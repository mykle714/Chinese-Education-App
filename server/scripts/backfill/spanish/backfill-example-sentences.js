...
const { stampEntries } = initRunLog({ script: 'spanish/backfill-example-sentences', version: SCRIPT_VERSION, anthropic: anthropic });
...
async function validatorAgent(word, pronunciation, definitionText, sentences) {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 900,
    temperature: 0.1,
    system: 'You are a strict native-Spanish reviewer of example sentences for a Spanish learning app. You catch subtle grammatical and naturalness errors that automated generation tends to make. Respond only with valid JSON.',
    messages: [{
      role: 'user',
