/**
 * Backfill Script: AI-powered vernacular register scoring for dictionaryentries_es (SPANISH)
 *
...
/**
 * Ask Claude Sonnet to score the vernacular register of a Spanish word.
 *
...
async function askClaudeForVernacularScore(word, pronunciation, definitions) {
  ...
  const header = `You are a Spanish linguistics expert specializing in sociolinguistics and register.
...
This is a register score — does this word live primarily in casual everyday speech (score high), or in written, formal, or literary contexts (score low)? The question is not whether the word is common or well-known, but whether it sounds natural and at home in everyday spoken Spanish.
  ...
  try {
    ...
    for (const row of entries) {
      try {
        ...
        await client.query(
          `UPDATE dictionaryentries_es SET "vernacularScore" = $1 WHERE id = $2`,
          [result.score, row.id]
        );
