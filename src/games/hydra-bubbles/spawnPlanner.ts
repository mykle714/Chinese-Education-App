...
import { rollColor, spawnWeightsAt, type Rng } from "./spawnTable";
import { NEW_CARD_SHARES, SHARES_PER_UNMATCHED_ROUND } from "./constants";
...
export function hasLiveMatch(pairs: readonly HydraBoardPair[]): boolean {
    return pairs.some((p) => p.hasWord && p.hasDefinition);
}
...
function forceLiveMatch(sim: Sim, rng: Rng, plannedId: string): HydraSpawnAction {
    ...
    if (strays.length > 0) {
        const stray = strays[Math.floor(rng() * strays.length)];
        return {
            type: "complete",
            pairId: stray.pairId,
