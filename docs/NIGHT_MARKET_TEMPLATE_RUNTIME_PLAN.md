...
> free placeholder UNIT slots one per unlock → spawn the next template via the anchor algorithm
> **only when an unlock finds no free slot**, that same unlock then filling the new template's
> first slot), and the hourly cron decays occupants back down when minutes are debited. Verified end-to-end on dev
> (fill hub → spawn 2nd hub on the south seam → recompute-on-read loads both). Filled slots now
> render a **placeholder occupant marker** — one house per filled UNIT slot (a 4×10/10×4 area is
> two units, each unlocked separately)
...
3. **Study 1 min → 1 unlock.** The grant pass takes the free island `ensureFreeSlot` hands it
   (inside the hub) and drops a stand occupant (recorded on `nightmarketunlocks` with
   `placedTemplateId` + `placeholderAreaId`). The hub may re-select `activeVersion` (an access
   street opens). Next render recomputes the graph; peds route to the new stand.
4. **Hub full → the next unlock triggers a spawn (lazy).** An unlock arrives, `ensureFreeSlot`
   finds nothing free, so `spawnTemplate` runs: enumerate exposed street anchors, pick closest to
   origin, look up matching-width catalog templates via load-time `anchorIndex`, score by
   street-runs joined, maximin-spread + random tiebreak, persist the new placement. The market
   grows a new block with streets continuous across the seam, and the unlock that triggered the
   growth fills that block's FIRST unit slot; its remaining units light up one per later unlock.
   Nothing is ever placed before an unlock needs it.
...
| `NightMarketPlacementService` | 3 | ✅ | WRITE/spawn path. `findFreeSlot` (read-only occupancy probe, iterating UNIT slots — one unlock per occupant footprint), `placeUnlock` (fill first free slot, no growth), `ensureFreeSlot` (guarantee a vacancy, spawning until one exists), `spawnTemplate` (full anchor algorithm via `templatePlacement.ts`), `grantUnlocks` (idempotent reconcile to `unlocks(m)`, **lazy spawn**: loop of ensure-vacancy → fill, nothing after the loop, so a template is placed only when an unlock has nowhere to go — see NIGHT_MARKET_TEMPLATES.md § "Lazy spawn"). Occupants tagged with a generic assetId (real stand-asset catalog + occupant→stand render is a later visual slice). Verified end-to-end on dev (fill 4 hub slots → spawn 2nd hub on the south seam → fill). |
...
| Dangling-template prune on decay | 4 | ✅ | **Reverses the append-only rule.** On any decay, empty (0-occupant) + weakly-attached placements are pruned to a fixpoint: removable ⟺ not-hub AND `!(hasEast&&hasWest) && !(hasHigh&&hasLow)` ({0,1,2-**adjacent**} sides; never a 2-**opposite** bridge). Pure core `server/dal/shared/templatePrune.ts#prunableDanglingPlacements` (6 scenarios verified); wrapper `NightMarketPlacementService.pruneDanglingTemplates` + `INightMarketPlacementDAL.deletePlacements` (occupants cascade). Triggers: author −N via `reconcileUnlocks` (decay branch); scheduled via compiled companion `dist/scripts/night-market/prune-dangling-templates.js` (second `ExecStart` of the `cow-maintenance` systemd user unit, ordered after the penalty SQL, targets `lastPenaltyDate=today`), wired into `install-maintenance-timer.sh`. Known edge: an empty L-connector (2 adjacent) to an occupied piece is removable → orphans it (accepted vs full connectivity guard). End-to-end run on dev user (6 placements, 0 removed = correct). |
...
| Author minute-adjust tool (nmp ±N buttons) | 4 | ✅ | `UserMinutePointsService.adjustMinutesForAuthor` + `POST /api/nightMarket/dev/adjustMinutes` (author-gated, 403). +N → `minutesEarned`+net; −N → `penaltyMinutes` (gross intact) + net debit floored; then `reconcileUnlocks`. Client: nmp `−6h/−1h/−1 · pending · +1/+1h/+6h · Submit` (steps are 1/60/360 MINUTES; the hour-sized ones are labelled in hours — `AUTHOR_MINUTE_STEPS` in `NightMarketEnginePage.tsx`) (gated on `isTemplateAuthor`), accumulates a signed delta, Submit posts once → updates the badge (net) + bumps a `reloadToken` that re-fetches the layout (`useMarketWorld(reloadToken)` → `MarketEngineViewer`) so occupants redraw. Verified end-to-end on dev (403 gate; +30 grant→17 occ; −50 penalty→9 occ, gross intact; +12→12 occ). |
| GROSS/NET display split | 4 | ✅ | `getLanguageSummary` now returns global NET (`users.totalMinutePoints`) + global GROSS (`users.lifetimeMinutesEarned`; was the on-read aggregate `getGrossMinutesEarned` until migration 133 turned it into a maintained counter — the API field is now `lifetimeMinutesEarned`). `useMinutePoints` exposes both; tester `TimeDisplay` shows big = NET ("Current Balance", drops on penalty) / small = GROSS ("total earned"); nmp badge + unlock check use NET (also fixed a latent client/server unlock-driver mismatch). See docs/MINUTE_POINTS_SYSTEM.md. |
