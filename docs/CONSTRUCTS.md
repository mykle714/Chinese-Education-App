...
- **Single-char zh**: tab is relabeled **"Used In"** and rendered by the shared `UsedInPaginatedList` (`src/features/flashcards/UsedInPaginatedList.tsx`, also used by the cdp "Used In" section). It **seeds** from `currentEntry.usedIn` (the 4-item preview) and then **infinite-scrolls** the remaining multi-char words via `GET /api/dictionary/used-in?character=&offset=&limit=` (offset = running item count, so pages continue seamlessly after the preview; `hasMore` comes from a fetch-one-extra probe). `vocabEntryId === null` flags a det-fallback item (not in the user's vet). The expansion block still renders below the rows when present.
