#!/usr/bin/env bash
#
# Run a backfill script from the HOST against the PPE database.
#
# LAYER: data-enrichment (backfill) invocation shim.
#
# WHY THIS EXISTS: backfills used to run inside `cow-backend` on the dev box
# and reach PPE only via a dev → PPE data-deploy push. That flow is retired — enrichment now runs
# directly against PPE. But the PPE backend image (`cow-ppe-backend`) ships
# neither scripts/backfill/ nor tsx, so `docker exec cow-backend ...` cannot
# work. Instead we run the repo's scripts on the host (node + server/node_modules
# are present) and point them at cow-postgres, which publishes 5432 on
# 127.0.0.1 only.
#
...
# The PPE password lives in the repo-root .env as POSTGRES_PASSWORD. Because
...
#
# USAGE:
#   scripts/backfill/run-ppe.sh scripts/backfill/chinese/backfill-hsk-level.js --words=未来
#   BACKFILL_ORACLE=export scripts/backfill/run-ppe.sh scripts/backfill/chinese/backfill-hsk-level.js
#
...
if [[ ! -f "$REPO_ROOT/.env" ]]; then
  echo "❌ $REPO_ROOT/.env not found — it carries POSTGRES_PASSWORD for the PPE DB." >&2
  exit 1
fi
...
# Point the scripts at the published PPE port rather than the compose hostname.
...
# TLS off: cow-postgres runs postgres:15-alpine, which is not built with TLS
