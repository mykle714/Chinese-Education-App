...
code, `docker-compose.yml` / `docker-compose.ppe.yml`, `database/migrations/`
...
|---|---|---|
| `machineEnvironment.md` | Claude / any agent working in the repo | Machine identity (`DEV` vs `PPE`); see [CLAUDE.md](../CLAUDE.md) |
...
`docker-compose.ppe.yml` env vars are documented separately in
[DOCKER_GUIDE.md](./DOCKER_GUIDE.md#environment-variables) and are only
relevant on the PPE server, not a second dev machine.
...
4. `docker-compose up --build -d` — brings up `cow-postgres` (host
   `5433`), `cow-backend` (host `5001` → container `5000`),
   `cow-frontend` (host `3000`), `cow-adminer` (host `8080`). Schema
...
the dump is plain-text) against `cow-postgres`, table by table. Confirm
