# Gloss Groups Push (dev → PPE, `gloss_meaning_groups`)

Push the GPU-pipeline-computed `gloss_meaning_groups` table from the dev box up to PPE.
This is the **one table in the app whose source of truth is DEV**, inverting the
ppe-is-authoritative rule every other data table follows — see
[`/data-ppe-to-dev`](./data-ppe-to-dev.md)'s own ⛔ warning against ever pulling this
...
**Direction is dev → PPE, always.** There is no counterpart flow. Never restore a
`gloss_meaning_groups` dump into `cow-postgres` from anywhere but the pipeline
itself running on dev.

> Structurally this mirrors [`/data-ppe-to-dev`](./data-ppe-to-dev.md) with the halves
> swapped: here dev is SOURCE and PPE is TARGET.
...
`gloss_meaning_groups` is empty on PPE — every `glossKeyToGroup` lookup misses, so
...
Read [machineEnvironment.md](../../machineEnvironment.md) (gitignored, present on every
machine).

- **On the DEV/GPU-pipeline box** → you are the SOURCE. Run every step below yourself.
- **On PPE** → you have no work to do here. You cannot run the pipeline or reach the dev
  Postgres instance from PPE; hand the steps below to whoever is on the dev box, then
  return to this session to run [step 4](#4-verification-sql-run-against-ppe) against
  PPE once they confirm the push completed.
...
**1c. Dry run — reads dev only, never contacts PPE.**
...
provenance line, then `--dry-run: PPE not contacted, nothing written`.

## 2. The push

`push-groups.ts` is the only writer of PPE's copy of this table, and the only script in
the repo that writes PPE from dev — it is structurally scoped to this one table.

```bash
cd ~/vocabulary-app/server
PPE_DB_HOST=<host> PPE_DB_PASSWORD=<password> \
  npx tsx scripts/gloss-pipeline/push-groups.ts
```

Source `<host>`/`<password>` the same way [`/deploy`](./deploy.md) sources PPE DB
...
The write is `TRUNCATE` + batched `INSERT` in **one transaction**, so PPE readers keep
...
Migration 154 (the table itself) is already live on PPE. This push writes rows into an
...
## 4. Verification SQL (run against PPE)
...
```sql
-- 4c. Size alarm, re-derived on PPE. The largest real group in the 2026-08-24 build
...
| 1b `must-NOT-block wrong` > 0 | **Do not push.** Investigate on dev — a wrong-block is a gameplay regression, and there is no hurry since PPE is unaffected either way. |
| Push throws mid-write | Nothing was committed; the transaction rolled back. PPE still holds the previous snapshot (or is still empty). Fix and re-run. |
| 4a shows >1 provenance row | Re-run the push. If it persists, `TRUNCATE gloss_meaning_groups;` on PPE and push again. |
...
here, and PPE never authors a row.
...
- ❌ **Do not add `gloss_meaning_groups` to `/data-ppe-to-dev`'s table list.** Its source
  of truth is dev — pulling it down overwrites dev's freshly computed groups with PPE's
  copy of what dev just sent up, silently.
- ❌ **Do not create `gloss_vectors` / `gloss_pair_verdicts` on PPE.** They are dev-only
  build caches (`dev-tables.sql`), deliberately not migrations. PPE runs no model and
  needs no GPU.
- ❌ **Do not hand-edit rows on PPE.** The table is replaced wholesale on every push; a
...
- ❌ **Do not push a build whose `corpusSnapshot` you cannot tie to a `validate.py` run.**
  The stamp is the only thread from a grouping on PPE back to the evidence for it.
