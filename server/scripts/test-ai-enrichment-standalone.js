/**
 * AI Enrichment Quality Test Script - Standalone Version
 * 
 ...
 * 
 * This is a standalone version that connects directly to PostgreSQL.
 * 
 * Usage: 
 *   ANTHROPIC_API_KEY=sk-ant-xxx OPENAI_API_KEY=sk-xxx node server/scripts/test-ai-enrichment-standalone.js
 * 
 * Optional DB env vars (uses defaults if not provided):
 *   DB_HOST=localhost DB_PORT=5432 DB_NAME=cow_db DB_USER=cow_user DB_PASSWORD=your-password
 */
...
if (!ANTHROPIC_API_KEY || !OPENAI_API_KEY) {
  console.error('❌ Error: API keys required!');
  console.error('Set environment variables: ANTHROPIC_API_KEY and OPENAI_API_KEY');
  console.error('\nExample:');
  console.error('  ANTHROPIC_API_KEY=sk-ant-xxx OPENAI_API_KEY=sk-xxx node server/scripts/test-ai-enrichment-standalone.js');
  process.exit(1);
}
...
1. **SYNONYMS** (3-5 words, ONLY if high quality):
...
2. **WORD EXPANSION** (1 expanded form):
   - Expand each morpheme/character into its fuller, more explicit form
   - Examples:
     * 不知不觉 → 不知道不觉得
     * 违规 → 违反规矩
     * 早晚 → 早上晚上
     * 左右 → 左边右边
     * 国防 → 国家防御
   - If the word cannot be meaningfully expanded, return null
   - The expansion should feel natural and maintain the original meaning
...
- ONLY respond with the JSON structure above - NO explanations, NO comments, NO additional text`;
}
...
async function callAnthropic(model, prompt) {
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 4096,
    temperature: 0.7,
    system: "You are a helpful assistant that ONLY responds with valid JSON. Never include explanations, comments, or any text outside the JSON structure.",
    messages: [{
      role: 'user',
      content: prompt
    }]
 ...
 */
async function testModel(modelKey, client) {
 ...
 */
async function main() {
  console.log('\n🚀 AI Enrichment Quality Test (Standalone)');
  ...
  try {
    ...
    for (const modelKey of Object.keys(MODELS)) {
      results[modelKey] = await testModel(modelKey, client);
