/**
 * Streak-day helpers — ISOMORPHIC.
 *
 * This module is the single source of truth for the "streak day" boundary and is
 * imported by BOTH the Node server (server/utils/streakDate.ts) and the browser
 * client (src/utils/streakDay.ts). Keep it free of any Node- or browser-only
 * imports — it may only use the standard library (Date, Intl). Do NOT add
 * platform-specific dependencies here, or you will break one of the two bundles.
 *
