...
| **TTS** | `src/services/tts/` (`CloudTTSProvider`, `WebSpeechProvider`) | Cloud provider survives (it's a fetch). `WebSpeechProvider` → `expo-speech`. Playback is always 1.0× — the `timeStretch`/`soundtouchjs` module and the speech-rate feature it served were **deleted 2026-08-28**, so there is nothing to port. The two-sink passthrough/media split ([AUDIO_PLAYBACK.md](./AUDIO_PLAYBACK.md)) becomes **one `expo-audio` flag**, `playsInSilentMode` — a **negative-cost** port, since RN also exposes the `mixWithOthers` the web withholds, so the two behaviors stop being a forced trade | Low | Close |
...
| **WebAudio answer-feedback sound** | `src/services/audio/markArpeggio.ts` | `expo-audio` | Low — it already ships real audio files (five mono mp3s), so there is no synthesized-blip design left to undo; what ports is the decode cache, the ladder counter and the gesture-unlock rule | Close |
...
| TTS | ~5 | S | **2** | A design decision, not a port. Simpler since the `timeStretch` DSP was deleted (2026-08-28) |
...
| "`timeStretch` is WebAudio and has no RN equivalent" | **Moot.** `timeStretch` was deleted on 2026-08-28 with the speech-rate feature. Were a speed control ever wanted again, **`expo-audio`'s `setRate(rate, shouldCorrectPitch: true)`** gives native pitch-corrected stretching for free — an argument for doing it natively rather than reviving client-side DSP |
...
| Replacement map | `src/features/nightmarket/*.tsx`, `src/components/handwriting/*`, `src/services/tts/*`, `src/games/runtime/useSidewaysStage.ts`, `src/services/audio/markArpeggio.ts`, `src/hooks/{usePageSlide,useBlockZoom,useBlockEdgeSwipe}.ts`, `src/App.tsx`, `package.json` |
