...
const VoteButton: React.FC<{
  ...
  size?: "small" | "large";
}> = ({ design, voted, voteDeltas, language, onVoteChange, size = "large" }) => {
  ...
  const toggle = async (e: React.MouseEvent) => {
    ...
    try {
      if (next) await voteForDesign(design.ownerUserId, design.entryKey, language);
      else await unvoteDesign(design.ownerUserId, design.entryKey, language);
    } catch {
      onVoteChange(design, !next);
    } finally {
      setPending(false);
