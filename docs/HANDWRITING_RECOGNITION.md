...
|---|---|---|
| Presentation (entry) | "Practice Writing Me" button on the **eip** (definition tab's action bar, labelled variant), the **flp main flashcard** (front face, stacked above the audio icon — icon variant), and **cdp** (zh + **1–4 characters**) | `src/components/handwriting/PracticeWritingButton.tsx`; placed in `src/features/flashcards/FlashcardsLearnPage/InfoCardActionBar.tsx`, `src/features/flashcards/FlashcardsLearnPage/FlashCardSection.tsx` (ChineseBlock) + `src/features/flashcards/VocabCardDetailPage.tsx` |
