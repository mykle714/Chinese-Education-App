...
|---|---|---|
| Presentation (entry) | One component, three `appearance`s (zh + **1–4 characters** on all of them): `rail` — the **`Write it` pill on `WordToolsRail`**, above the card on the flp and the cdp, and the main entry point since 2026-08-24; `icon` — the compact button on the **dictionary cdp**'s card face; `labeled` — the plain "Practice Writing Me" button, for any future action row. The eip's own action bar (`InfoCardActionBar`) is **deleted** — the panel is information-only now, and practising is a word tool | `src/components/handwriting/PracticeWritingButton.tsx`; placed by `src/components/WordToolsRail.tsx` and `src/features/flashcards/card/CardFace.tsx` (ChineseBlock, dictionary cdp only) |
