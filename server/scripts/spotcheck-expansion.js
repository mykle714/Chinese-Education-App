/**
 * Spot-check script: test the updated expansion prompt against 50 random entries.
 * Reads-only — no DB writes.
 * Usage: docker exec cow-backend-local npx tsx scripts/spotcheck-expansion.js
 */
...
/**
 * Post-processing validation — mirrors DictionaryService.validateExpansion().
...
{"expansion": "expanded form"} or {"expansion": null}`;
  ...
  if (!objMatch) throw new Error(`No JSON object in response: ${content}`);
...
async function run() {
  ...
  try {
    ...
    console.log(`Spot-checking ${rows.length} entries with updated expansion prompt...\n`);
    ...
    for (const row of rows) {
      const defs = Array.isArray(row.definitions) ? row.definitions : JSON.parse(row.definitions || '[]');
      const defPreview = defs.slice(0, 2).join('; ');
      try {
        const result = await generateExpansion(row.word1);
        if (result) {
          console.log(`✅  ${row.word1} (${row.pronunciation}) → ${result}`);
          console.log(`    "${defPreview}"`);
          expanded++;
        } else {
          console.log(`—   ${row.word1} (${row.pronunciation}) → null`);
          console.log(`    "${defPreview}"`);
          nulled++;
        }
      } catch (err) {
        console.log(`❌  ${row.word1} (${row.pronunciation}) → ERROR: ${err.message}`);
        failed++;
      }
      console.log();
    }
    ...
    console.log('='.repeat(70));
    console.log(`Total: ${rows.length}  |  Expanded: ${expanded}  |  Null: ${nulled}  |  Errors: ${failed}`);
  } finally {
    client.release();
    await db.pool.end();
  }
