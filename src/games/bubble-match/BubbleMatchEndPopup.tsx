...
/**
 * End-of-run popup (won / lost) for Bubble Match with a minimize-to-corner
 * affordance.
 *
 * Layout layer: presentational. The page owns the `minimized` flag and feeds
 * back the card content; this component owns the scrim, the card chrome (the ×
 * button), the corner puck, and the FLIP-style collapse animation between them.
 *
 * The card stays flex-centered in its natural (un-transformed) size so its
 ...
 */
const BubbleMatchEndPopup: React.FC<BubbleMatchEndPopupProps> = ({
    minimized,
    onMinimize,
    onRestore,
    children,
    ...
    return (
        ...
        >
            ...
            >
                ...
                <IconButton
                    className="bubble-match__popup-close"
                    aria-label="Minimize"
                    onClick={onMinimize}
                    size="small"
                    sx={{
            ...
            {/* Minimized puck — fades in as the card collapses; click to restore.
                ...
                a small visual square: because the scrim is pointer-events:none when
                minimized, a tap just outside a 52px target would otherwise fall
                through to a bubble and trigger study-select (audible/visible). The
                padded hit pad (pointer-events:auto) catches close taps in a comfy
                radius and shields the bubbles directly beneath it. */}
            <Box
                className="bubble-match__popup-puck-hit"
                role="button"
                aria-label="Reopen menu"
                onClick={onRestore}
                // Belt-and-suspenders: keep the close gesture from bubbling anywhere.
            ...
            >
                <Box
                    className="bubble-match__popup-puck"
                    sx={{
                        width: SQUARE_SIZE,
                        height: SQUARE_SIZE,
                        borderRadius: "14px",
