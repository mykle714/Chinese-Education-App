      ...
      try {
        ...
        if (rescoreOnly) {
          ...
          for (const cluster of existing) {
            ...
            try {
              // `pos` feeds the sense-mode GRAMMATICAL-FUNCTION guideline (v10).
              const s = await scoreFrequency(row.word1, cluster.reading, glosses, { sense: cluster.sense, pos: toPosArray(cluster.pos) });
