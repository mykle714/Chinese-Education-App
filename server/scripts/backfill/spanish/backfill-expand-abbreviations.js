/**
 * Backfill Script: Expand lexicographic abbreviations for dictionaryentries_es (SPANISH)
 *
 * Spanish counterpart of backfill/chinese/backfill-expand-abbreviations.js.
 * Deterministic string op (no AI). Definitions are English glosses, so the same
 * "sth"→"something" / "sb"→"somebody" expansions apply; only the target table
 * differs.
 *
 * Usage:
 *   docker exec cow-backend-local npx tsx scripts/backfill/spanish/backfill-expand-abbreviations.js [--dry-run]
 */
import dotenv from 'dotenv';
import { fileURLToPath } from 'url';
import path from 'path';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
dotenv.config({ path: path.join(__dirname, '../../../.env.docker') });

import db from '../../../db.js';
import { initRunLog } from '../run-log.js';
const SCRIPT_VERSION = 1; // bump when this script's logic/prompt changes
// run-log: track duration, version, and words/mode
const { stampEntries, validatedClause } = initRunLog({ script: 'spanish/backfill-expand-abbreviations', version: SCRIPT_VERSION });

const DRY_RUN = process.argv.includes('--dry-run');

/**
 * Expands common lexicographic abbreviations in a definition string:
 *   - "sth" → "something"  (e.g. "to do sth", "(sth)", "of sth")
 *   - "sb"  → "somebody"   (e.g. "to give sb sth", "(sb)", "sb's" → "somebody's")
 *
 * Uses word boundaries (\b) so partial matches inside longer words are not affected.
 * Order matters: replace "sth" before "sb" to avoid any edge-case overlap.
 */
function expandAbbreviations(def) {
  return def
    .replace(/\bsth\b/g, 'something')
    .replace(/\bsb\b/g, 'somebody');
}

async function backfillExpandAbbreviations() {
  console.log(`Starting abbreviation-expansion backfill... ${DRY_RUN ? '(DRY RUN — no writes)' : ''}\n`);

  const client = await db.getClient();

  try {
    // Skip rows whose `definitions` bundle a validator has already approved/flagged
    // — this pass rewrites `definitions` in place, so without the guard a re-run
    // would silently clobber human-reviewed text (docs/DATA_VALIDATION_SYSTEM.md,
    // "Backfill guard"). Unlike the AI steps this query is deliberately table-wide
    // (no discoverable filter), which is exactly why it needs the guard.
    const result = await client.query(`
      SELECT id, word1, definitions
      FROM dictionaryentries_es
      WHERE language = 'es'
        AND ${validatedClause(['definitions'], 'dictionaryentries_es')}
      ORDER BY id
    `);

    const entries = result.rows;
    console.log(`Found ${entries.length} total entries\n`);

    let changedCount = 0;
    let unchangedCount = 0;

    for (const entry of entries) {
      const definitions = Array.isArray(entry.definitions)
        ? entry.definitions
        : JSON.parse(entry.definitions || '[]');

      const expanded = definitions.map(expandAbbreviations);

      // Only update if something actually changed
      if (JSON.stringify(definitions) === JSON.stringify(expanded)) {
        unchangedCount++;
        continue;
      }

      changedCount++;
      console.log(`[${entry.id}] "${entry.word1}"`);
      console.log(`  Before: ${JSON.stringify(definitions)}`);
      console.log(`  After:  ${JSON.stringify(expanded)}`);

      if (!DRY_RUN) {
        await client.query(`
          UPDATE dictionaryentries_es
          SET definitions = $1::jsonb
          WHERE id = $2
        `, [JSON.stringify(expanded), entry.id]);
        await stampEntries(client, 'dictionaryentries_es', entry.id);
      }
    }

    console.log('\n' + '='.repeat(60));
    console.log('BACKFILL SUMMARY');
    console.log('='.repeat(60));
    console.log(`Total entries scanned: ${entries.length}`);
    console.log(`Entries changed:       ${changedCount}`);
    console.log(`Entries unchanged:     ${unchangedCount}`);
    if (DRY_RUN) console.log('\n(DRY RUN — no changes were written to the database)');
    console.log('='.repeat(60));
    console.log('\nBackfill complete.');

  } catch (error) {
    console.error('Fatal error during backfill:', error);
    throw error;
  } finally {
    client.release();
    await db.pool.end();
  }
}

backfillExpandAbbreviations().catch(error => {
  console.error('Fatal error:', error);
  process.exit(1);
});
